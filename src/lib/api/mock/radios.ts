...
const stationsDatabase: Omit<RadioStation, 'activityLevel' | 'lastActivity' | 'history'>[] = [
  {
    id: 'radio_uvb76',
...
function generateStationData(station: (typeof stationsDatabase)[number]): RadioStation {
  ...
  const history = Array.from({ length: 5 }, (_, i) => ({
    timestamp: new Date(Date.now() - (i + 1) * Math.floor(historyRandom() * 86400000)).toISOString(),
