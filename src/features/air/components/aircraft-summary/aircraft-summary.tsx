...
export const AircraftSummary = ({
  aircraft,
  isLoading = false,
}: AircraftSummaryProps) => {
  if (isLoading) {
    return null;
  }

  const militaryCount = aircraft.filter(
    (a) => a.category === "military",
  ).length;
  const governmentCount = aircraft.filter(
    (a) => a.category === "government",
  ).length;
  const commercialCount = aircraft.filter(
    (a) => a.category === "commercial",
  ).length;
      ...
      <Card padding="lg">
        ...
        <div className={styles.grid}>
          ...
          <div className={styles.stat}>
            ...
            <span className={`${styles.statValue} ${styles.government}`}>
              {governmentCount}
            </span>
          </div>
          <div className={styles.stat}>
