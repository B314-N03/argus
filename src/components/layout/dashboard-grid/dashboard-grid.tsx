...
export function DashboardGrid({ children, variant = 'default', className = '' }: DashboardGridProps) {
  const gridClass = variant === 'grid2' ? styles.grid2 : variant === 'grid3' ? styles.grid3 : styles.grid
...
export function GridItem({ children, fullWidth = false, span, className = '' }: GridItemProps) {
  let spanClass = ''
